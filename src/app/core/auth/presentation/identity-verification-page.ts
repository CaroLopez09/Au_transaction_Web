      ...
      <section aria-labelledby="identity-title">
        ...
        @if (!challenge()) {
          ...
          <form [formGroup]="form" (ngSubmit)="submit()" novalidate>
            ...
            <label class="au-label" for="selfie">Rostro</label>
            <input id="selfie" type="file" accept="image/jpeg,image/png" capture="user" (change)="file('selfie', $event)" />
            ...
            <button type="submit" class="au-button au-button--primary" [disabled]="busy()">{{ busy() ? 'Validando...' : 'Enviar para validación' }}</button>
          </form>
        }
      </section>
    </main>
  `,
  styles: `.identity { max-width: 38rem; margin: 4rem auto; padding: 0 1.5rem; } form { display: grid; gap: .75rem; } h1 { margin: .25rem 0; } .eyebrow { color: var(--au-text-muted); } .consent { margin: .75rem 0; } .error { color: var(--au-critical, #b3261e); } .notice { color: var(--au-success, #216e39); }`,
})
export class IdentityVerificationPage {
  ...
  protected async submit(): Promise<void> {
    ...
    if (this.form.invalid || !images.front || !images.back || !images.selfie || this.busy()) {
      this.form.markAllAsTouched(); this.error.set('Adjunta las tres imágenes y acepta el consentimiento.'); return;
    }
